local inventory_manager = System({_components.inventory, "ALL"}, {
    _components.inventory, _components.controlled, "PLAYER"
})
...
function inventory_manager:init()
    INVENTORY_CELL_SIZE = (love.graphics.getWidth() *
                              (1 - INVENTORY_SCREEN_OFFSET_RATIO.x * 2) /
                              INVENTORY_SLOTS_PER_ROW)
    self.current_open_inventory = nil
end

function inventory_manager.resize(_, w, _)
    INVENTORY_CELL_SIZE = (w * (1 - INVENTORY_SCREEN_OFFSET_RATIO.x * 2) /
                              INVENTORY_SLOTS_PER_ROW)
end
...
function inventory_manager.action_held(_, _, _) end
...
function inventory_manager:toggle_inventory(action, entity)
    assert(action)
    assert(entity)
    if action ~= "toggle_inventory" then return end

    local inventory = entity:get(_components.inventory)
    if (self.current_open_inventory and self.current_open_inventory.id ==
        inventory.id) then
...
function inventory_manager.add_item_to_inventory(_, item, inventory)
    ...
    if target_slot then
        ...
        print(
            "Failed to add item to inventory. No appropriate inventory slot found")
        return false
    end
end
...
function inventory_manager:draw_ui()
    for i = 1, self.ALL.size do
        ...
        if self.current_open_inventory and self.current_open_inventory.id ==
            inventory.id then
            local offset = Vector(INVENTORY_SCREEN_OFFSET_RATIO.x *
                                      love.graphics.getWidth(),
                                  INVENTORY_SCREEN_OFFSET_RATIO.y *
            ...
            for j = 0, inventory.size - 1 do
                if j % INVENTORY_SLOTS_PER_ROW == 0 then
                    ...
                    offset.x = INVENTORY_SCREEN_OFFSET_RATIO.x *
                                   love.graphics.getWidth()
                end
                love.graphics.setColor(0.6, 0.6, 0.6, 1)
                love.graphics.setLineWidth(4)
                love.graphics.rectangle("line", offset.x, offset.y, cell_width,
                                        cell_width)
                love.graphics.setLineWidth(10)
                love.graphics.setColor(0.6, 0.6, 0.6, 0.8)
                love.graphics.rectangle("fill", offset.x, offset.y, cell_width,
                                        cell_width)
