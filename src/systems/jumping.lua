  ...
  for i = 1, self.ALL.size do
    ...
    if movement_state.behaviour.state == "walk" or movement_state.behaviour.state == "default" then
      local _, len_left =
        ...
        transform.position.y + collides.offset.y + collides.height * _constants.Y_OFFSET_TO_TEST_PLAYER_IS_GROUNDED,
        world_filter
      )
      ...
      if len_left == 0 and len_right == 0 and len_centre == 0 then
        ...
        if e:has(_components.controlled) and e:has(_components.direction) then
          ...
          e:get(_components.air_control).x_velocity =
            e:get(_components.walk).x_velocity * _constants.PLAYER_WALK_OFF_LEDGE_MOMENTUM_CONSERVATION_RATIO *
            held_modifier
        end
      end
    end
          ...
          if e:has(_components.air_control) then
            ...
            if e:has(_components.controlled) and e:has(_components.direction) then
              ...
              e:get(_components.walk).x_velocity =
                e:get(_components.air_control).x_velocity * _constants.PLAYER_AIR_TO_GROUND_MOMENTUM_CONSERVATION_RATIO *
                held_modifier
            end
          end
